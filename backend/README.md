...
后端使用 Java 17 + Spring Boot 开发，默认端口 `8080`。它是三端数据流转的核心，负责统一 API、MySQL 数据读写、账号认证、平台账号绑定、订单售后评价、聊天会话、微信登录、管理员治理和 AI 服务转发。

## 当前职责

- 一级账号登录、手机号验证码登录、微信登录、资料更新和封禁校验。
...
- 调用 AI 服务生成客服回复、知识库解析、单条评价分析和店铺评价总体分析。

## 主要模块

- `platform`：万象商城、悦购集市、本地模拟平台和管理员接口。
...
```powershell
$env:WECHAT_OPEN_APPID="网站应用 AppID"
$env:WECHAT_OPEN_SECRET="网站应用 AppSecret"
```
...
- `WECHAT_MINIAPP_APPID`、`WECHAT_MINIAPP_SECRET`：消费者小程序微信登录。
- `WECHAT_OPEN_APPID`、`WECHAT_OPEN_SECRET`：商家端扫码登录预留。
